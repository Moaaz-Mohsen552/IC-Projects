module FIFO (
    ...
    always @(posedge FIFO_if.clk or negedge FIFO_if.rst_n) begin
        if (!FIFO_if.rst_n) begin
            wr_ptr <= 0;
            FIFO_if.wr_ack <= 0;  // Reset write acknowledge
...
always_comb begin
    if (!FIFO_if.rst_n) begin
        reset_assertion: assert final((!wr_ptr) && (!rd_ptr) &&
                                      (!count)); // reset assertion
    end
end
...
always_comb begin
    ...
    almostfull_assertion: assert final(FIFO_if.almostfull == (count == FIFO_DEPTH-1));
    empty_assertion: assert final(FIFO_if.empty == (count == 0) ? 1 : 0);
